...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    const ActModal = document.querySelector('.Actmodal')
    const actCloser = document.querySelector('.actCloser')

    actCloser.addEventListener('click', () => {
    ...
    axios.get(`https://studysyncserver.onrender.com/getAnalytics/${user.uid}`)
      .then((res) => {
        const { data } = res;
        console.log('=========================');
        console.log(data);
        ...
        function drawChart() {
          // Get today's date and generate the last 7 days
          const today = new Date();
          const last7Days = Array.from({ length: 7 }, (_, i) => {
            const date = new Date(today);
            date.setDate(today.getDate() - i);
            return date.toLocaleDateString(); // Format as locale date string
          }).reverse(); // Ensure the dates are in ascending order
          ...
          data.forEach((item) => {
            ...
            if (!groupedData[date]) {
              groupedData[date] = { Find: 0, Schedule: 0 };
            }
            groupedData[date][item.Activity] += 1; // Increment activity count
          });
        
          // Prepare chart data with all days (fill missing days with 0)
          const chartDataArray = [['Date', 'Find', 'Schedule']];
          last7Days.forEach((date) => {
            const findCount = groupedData[date]?.Find || 0;
            const scheduleCount = groupedData[date]?.Schedule || 0;
            chartDataArray.push([date, findCount, scheduleCount]);
          });
          ...
          const options = {
            chart: {
              title: 'User Activity',
              subtitle: 'Find and Schedule activities over the last 7 days',
            },
    ...
    console.log(user)
    axios.post('https://studysyncserver.onrender.com/getActivity', { Uid: user.uid })
      .then((res) => {
        const { data } = res
